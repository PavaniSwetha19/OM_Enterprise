/**
 * Xpressbees Shipping API Service Utility for Om Enterprises
...
export const CONSIGNER_DATA = {
  consigner_name: "Om Enterprises Co",
  ...
  try {
    ...
    const payload = {
      ...
      products: items.map((item: XpressbeesItem) => ({
        product_name: item.productName || item.name || "Om Enterprises Fit Product",
